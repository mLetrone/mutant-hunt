...
import type { Percent, SilverCoin } from "../money";
...
const RANK_DISCOUNT: Readonly<Record<GuildRank, Percent>> = {
  novice: percent(0),
...
const VOLUME_DISCOUNT_LOW_RATE: Percent = percent(5);
const VOLUME_DISCOUNT_HIGH_THRESHOLD = 50;
const VOLUME_DISCOUNT_HIGH_RATE: Percent = percent(12);
 ...
 */
const LUNAR_SURCHARGE: Readonly<Record<MoonPhase, Percent>> = {
  "new-moon": percent(0),
 ...
 * of its subtotal.
 */
const DISCOUNT_CAP: Percent = percent(25);
...
export class Bill {
  private constructor(
    ...
    private readonly rankRateValue: Percent,
    private readonly volumeRateValue: Percent,
    private readonly discountRateValue: Percent,
  ...
  public static from(order: Order): Bill {
    ...
    const rankRate: Percent = Bill.computeRankRate(order.guildRank, order.lineCount);
    const volumeRate: Percent = Bill.computeVolumeRate(order.totalQuantity);
    const discountRate: Percent = Bill.capDiscountRate(rankRate, volumeRate);
  ...
  private static priceLine(sku: Sku, quantity: number, moonPhase: MoonPhase): BilledLine {
    ...
    const lunarRate: Percent = item.isSilverPlated ? LUNAR_SURCHARGE[moonPhase] : percent(0);
  ...
  private static computeRankRate(rank: GuildRank, lineCount: number): Percent {
  ...
  private static computeVolumeRate(totalQuantity: number): Percent {
  ...
  private static capDiscountRate(rankRate: Percent, volumeRate: Percent): Percent {
    const combined: number = rankRate + volumeRate;
    return (combined > DISCOUNT_CAP ? DISCOUNT_CAP : combined) as Percent;
  }
}
